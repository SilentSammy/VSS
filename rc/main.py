...
class MecanumBLEClient:
    ...
    async def set_x(self, value):
        ...
        await self.client.write_gatt_char(self.uuid_x, bytes([to_byte(value)]))
    
    async def set_y(self, value):
        """Set Y-axis (strafe) value (-1.0 to 1.0)"""
        await self.client.write_gatt_char(self.uuid_y, bytes([to_byte(value)]))
    
    async def set_w(self, value):
        """Set W-axis (rotation) value (-1.0 to 1.0)"""
        await self.client.write_gatt_char(self.uuid_w, bytes([to_byte(value)]))
